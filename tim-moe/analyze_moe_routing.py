"""
Analysis for the MoE router probe (tim-moe/moe_router_probe.py).

Reads tim-moe/logs/moe_probe/routing_raw.json and answers two falsifiable
questions per layer, then overall:
...
ROUTING_RAW = TIM_MOE_DIR / "logs" / "moe_probe" / "routing_raw.json"
FIG_DIR = TIM_MOE_DIR / "docs" / "figures"
FIG_DIR.mkdir(parents=True, exist_ok=True)
OUT_PATH = TIM_MOE_DIR / "logs" / "moe_probe" / "analysis_summary.json"
...
def make_router_shift_figure(q1, layers):
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "moe_router_shift.png", dpi=150)
    plt.close(fig)
...
def make_prediction_overlap_figure(q2, layers):
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "moe_prediction_overlap.png", dpi=150)
    plt.close(fig)
...
def main():
    ...
    print(f"\n[write] {OUT_PATH}")
    print(f"[write] {FIG_DIR / 'moe_router_shift.png'}")
    print(f"[write] {FIG_DIR / 'moe_prediction_overlap.png'}")
