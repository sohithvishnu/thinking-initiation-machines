...
# Domain seed vocabulary — real Python syntax and idioms, not a descriptive
# sentence about Python. Built from the standard library so it's reproducible
# and not hand-curated/easy to get subtly wrong.
#   - keyword.kwlist: def, return, if, for, class, import, etc. (all real
#     Python reserved words for the running interpreter's version)
#   - dir(builtins): len, range, print, list, dict, str, isinstance, etc.
#   - common dunder/OOP patterns and operators not otherwise captured
PYTHON_DOMAIN_WORDS = (
    list(keyword.kwlist)
...
# Persona text used for the prompt_control condition (kept separate from
# the KV-injection domain pool above — this is plain English, delivered
# conventionally in the prompt, not tokenized into noise).
DOMAIN_PERSONA_TEXT = (
...
def run_model(model_id: str, args) -> dict:
    ...
    domain_token_ids = primer.get_domain_token_ids_from_words(PYTHON_DOMAIN_WORDS)
