...
`experiment-3/run_int4_tim_domain.py` imports model loading, determinism
checking, and the timed `run_condition` runner from
`experiment-3/run_quant_gap.py` rather than reimplementing them.
...
same NF4 config and the same `experiment-3` code path (the gate script and
`run_int4_tim_domain.py` share `load_model_with_quant`).
...
`experiment-2/tim_primer.py`, `PYTHON_DOMAIN_WORDS` (the 211-word pool built
from `keyword.kwlist` + `dir(builtins)`, same pool `run_quant_gap.py` and
the bf16 study both use), seeds 0/1/2, per-task `copy.deepcopy` cache
...
- `experiment-3/run_int4_tim_domain.py` — generates the missing cell
  (imports model loading / determinism / `run_condition` from
  `experiment-3/run_quant_gap.py`).
...
- **bf16 tim_domain has no timing/memory instrumentation.** It was
  generated by the original `tim.py` (root), which predates the
  timing/peak-memory instrumentation added in `experiment-2/exp-2.py` and
  `experiment-3/run_quant_gap.py`. Only `pass@1_base` / `pass@1_base_plus_extra`
